 ...
 *
 * Ported from ../frontend/src/meshes/FurnitureFactory.ts
 */

const meshes: Map<FurnitureType, THREE.Object3D> = new Map();

let initialized = false;

export function initializeFurnitureFactory(): void {
  if (initialized) return;
  initialized = true;

  meshes.set('COMPUTER_DESK', createComputerDesk());
  meshes.set('COUCH_2_SEATER', createCouch2Seater());
  meshes.set('COUCH_SINGLE', createCouchSingle());
  meshes.set('BAR_STOOL', createBarStool());
}

export function getFurnitureMesh(type: FurnitureType): THREE.Object3D | null {
  const original = meshes.get(type);
  return original ? original.clone() : null;
}

// ─── Furniture constructors ──────────────────────────────────────────────────

function createComputerDesk(): THREE.Object3D {
...
function createCouch2Seater(): THREE.Object3D {
  const group = new THREE.Group();
...
function createCouchSingle(): THREE.Object3D {
  const group = new THREE.Group();
...
function createBarStool(): THREE.Object3D {
  const group = new THREE.Group();
