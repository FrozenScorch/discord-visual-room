...
exports.generateLinearLayout = generateLinearLayout;
exports.generateCircularLayout = generateCircularLayout;
exports.generateMixedLayout = generateMixedLayout;
exports.generateActivityBasedLayout = generateActivityBasedLayout;
 ...
 */
function generateLinearLayout(count, config = {}) {
 ...
 * @returns Array of furniture assignments with positions
 */
function generateCircularLayout(count, radius = 5) {
    const assignments = [];
 ...
 * @returns Array of furniture assignments with positions
 */
function generateMixedLayout(count) {
    const baseLayout = generateLinearLayout(count);
    const furnitureTypes = types_js_1.VALID_FURNITURE_TYPES;
    return baseLayout.map((assignment, index) => ({
        ...assignment,
        // Rotate through furniture types deterministically
        furniture: furnitureTypes[index % furnitureTypes.length]
    }));
}
/**
 ...
 * @returns Array of furniture assignments with positions
 */
function generateActivityBasedLayout(count, activityType) {
 ...
 */
function assignPositions(assignments) {
    const layout = generateLinearLayout(assignments.length);
    return assignments.map((assignment, index) => ({
        userId: assignment.userId,
        furniture: assignment.furniture,
