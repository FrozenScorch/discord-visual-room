...
exports.validateFurnitureTypes = validateFurnitureTypes;
 ...
 */
function validateLLMResponse(response, userIds) {
    const errors = [];
    ...
    for (let i = 0; i < parsed.length; i++) {
        ...
        if (!(0, types_js_1.isValidFurnitureType)(furniture)) {
            errors.push(`Assignment ${i} has invalid furniture type: "${furniture}". ` +
                `Valid types are: ${types_js_1.VALID_FURNITURE_TYPES.join(", ")}`);
            continue;
        }
 ...
 */
function validateFurnitureTypes(assignments) {
    const errors = [];
 ...
 */
function needsCleaning(response) {
